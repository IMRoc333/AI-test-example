...
2. 知识召回：基于向量检索召回业务规则、历史缺陷和测试经验。
3. 结构化生成：约束模型输出统一 JSON 测试用例。
4. 跑偏防护：PRD 是唯一被测对象，知识库只作为参考。
5. 质量评估：从覆盖率、逻辑一致性、重复度、缺失场景等维度评分。
6. Agent 优化：根据评估反馈多轮补齐测试场景。

适合在简历中描述为：

> 基于 RAG 增强与 LLM-as-Judge 反馈机制的 PRD 测试用例自动生成平台，支持 OpenAI-compatible 国产大模型接入、多模态 PRD 解析、知识库召回、质量评估与 Agent 式迭代优化。
