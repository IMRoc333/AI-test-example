...
- 待确认问题澄清：用户可回答 AI 识别出的需求疑点，并带入后续生成
- 测试用例溯源：每条用例标注来源与来源说明，减少 AI 编造风险
...
4. 需求澄清：用户回答 AI 识别出的待确认问题，澄清答案会合并进生成上下文。
5. 人工确认：用户确认或编辑模块树后再生成用例，避免 AI 直出跑偏。
6. 知识召回：基于向量检索召回业务规则、历史缺陷和测试经验。
7. 用例生成 Agent：基于确认后的模块树生成结构化 JSON 测试用例，并标注来源。
8. 用例溯源：每条用例记录 source 和 source_detail，可追溯到 PRD、用户澄清、知识库、历史缺陷或 AI 风险推理。
9. 跑偏防护：PRD 是唯一被测对象，知识库只作为参考。
10. 评估优化 Agent：从覆盖率、逻辑一致性、重复度、缺失场景、来源可追溯性等维度评分并迭代修正。
11. 规则沉淀 Agent：从优化结果中提炼可复用测试规则，并由用户确认保存到知识库。

适合在简历中描述为：

> 参考快手智能测试用例生成系统的多阶段演进思路，设计并实现了基于 RAG 增强和 Multi-Agent 协作的 PRD 测试用例生成平台。系统将非规范需求处理拆分为文档解析 Agent、模块生成 Agent、用例生成 Agent、评估优化 Agent 和规则沉淀 Agent，支持 OpenAI-compatible 国产大模型、多模态 PRD 解析、人工确认模块树、需求澄清、用例溯源、知识库召回和 LLM-as-Judge 质量评估，解决直接调用大模型时业务理解不足、输出不稳定、来源不可解释和覆盖不完整的问题。
